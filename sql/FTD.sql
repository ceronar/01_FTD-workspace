...
DROP TABLE "INIQUIRY_FILE"; // 문의 파일
...
CREATE TABLE "SELLER" (
	...
	"SELLER_PWD"	    VARCHAR2(25)		NOT NULL,
...
COMMENT ON COLUMN "GOODS_ORDER"."ORDER_DETAIL_NO" IS '주문 상세 번호';
COMMENT ON COLUMN "GOODS_ORDER"."ORDER_NO" IS '주문 번호';
COMMENT ON COLUMN "GOODS_ORDER"."GOOD_NO" IS '상품 번호';
COMMENT ON COLUMN "GOODS_ORDER"."COUNT" IS '수량';
COMMENT ON COLUMN "GOODS_ORDER"."PRICE" IS '상품별 가격';
COMMENT ON COLUMN "GOODS_ORDER"."STATUS" IS '주문 처리 상태';
...
COMMENT ON COLUMN "INIQUIRY_FILE"."FILE_NO" IS '파일 번호';
COMMENT ON COLUMN "INIQUIRY_FILE"."INQ_NO" IS '문의 번호';
COMMENT ON COLUMN "INIQUIRY_FILE"."ORIGIN_NAME" IS '원본명';
COMMENT ON COLUMN "INIQUIRY_FILE"."CHANGE_NAME" IS '수정명';
COMMENT ON COLUMN "INIQUIRY_FILE"."CREATE_DATE" IS '등록일';
COMMENT ON COLUMN "INIQUIRY_FILE"."DELETE_DATE" IS '삭제일';
...
ALTER TABLE "GOODS_ORDER" ADD CONSTRAINT "PK_GOODS_ORDER" PRIMARY KEY (
	"ORDER_DETAIL_NO"
);
...
ALTER TABLE "INIQUIRY_FILE" ADD CONSTRAINT "PK_INIQUIRY_FILE" PRIMARY KEY (
	"FILE_NO"
);
...
ALTER TABLE "GOODS_ORDER" ADD CONSTRAINT "FK_ORDER_TO_GOODS_ORDER_1" FOREIGN KEY (
	"ORDER_NO"
)
REFERENCES "ORDER" (
	"ORDER_NO"
...
ALTER TABLE "GOODS_ORDER" ADD CONSTRAINT "FK_GOODS_TO_GOODS_ORDER_1" FOREIGN KEY (
	"GOOD_NO"
)
REFERENCES "GOODS" (
	"GOOD_NO"
...
ALTER TABLE "INIQUIRY_FILE" ADD CONSTRAINT "FK_INQUIRY_TO_INIQUIRY_FILE_1" FOREIGN KEY (
	"INQ_NO"
)
REFERENCES "INQUIRY" (
	"INQ_NO"
